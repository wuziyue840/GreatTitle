...
/**
 * 简单的显示管理器 - 无需任何依赖，使用原生Minecraft功能
 * 核心原理：Scoreboard Team + Player DisplayName
 */
public class SimpleDisplayManager {
    ...
    public SimpleDisplayManager(GreatTitle plugin) {
        this.plugin = plugin;
    ...
    private void updateTeamPrefix(Player player, String titleContent) {
        try {
            ...
            // 设置前缀
            String prefix = formatText(titleContent, 64);
    ...
    private void updateInlineDisplay(Player player, String titleContent) {
        ...
        displayName = formatText(displayName, 32);
    ...
    public void setDebugEnabled(boolean enabled) {
        ...
        debug("简单显示管理器调试模式: " + (enabled ? "开启" : "关闭"));
    }
    ...
    private void debug(String message) {
        if (debugEnabled) {
            plugin.getLogger().info("[显示调试] " + message);
        }
    }
}